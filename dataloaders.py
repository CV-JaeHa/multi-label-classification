...
───── multi-label-classification
     ├── model
     └── data
          ├── train_data (before : dirty_mnist)
          │   └── 00000~49999.png
          └── test_data (before : test_dirty_mnist)
               └── 50000~54999.png
"""
os.chdir("./")
ROOT_PATH = os.getcwd()
...
"""
if os.path.isfile(data_path + '/Train_Img_path.pkl'):
    with open(data_path + '/Train_Img_path.pkl', 'rb') as f:
        imgs_path = pickle.load(f)
else:
    imgs_path = np.array(sorted(glob.glob(os.path.join(train_data_path, '*.png'))))
    with open(data_path + '/Train_Img_Path.pkl', 'wb') as f:
...
"""
if os.path.isfile(data_path + '/Imgs_numpy.npy'):
    st = time.time()
    imgs = np.load(data_path + '/Imgs_Numpy.npy')
    ...
    for img_file in tqdm(imgs_path):
        ...
        imgs.append(img)
    imgs = np.array(imgs)
    np.save(data_path + '/Imgs_Numpy.npy', imgs)
...
"""
if os.path.isfile(data_path + '/Train_KFold.pkl') :
    with open(data_path + '/Train_KFold.pkl', 'rb') as f :
        folds = pickle.load(f)
else:
    ...
    with open(data_path + '/Train_KFold.pkl', 'wb') as f:
        pickle.dump(folds, f)
